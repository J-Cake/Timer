	...
	get () {
		...
		return new Promise((res, rej) => {
			...
			this.resolve = (function () {
				...
				[...this.self.querySelectorAll('.value-output')].forEach(element => {
					parsedBodyContent[element.getAttribute('name') || "info" + Object.keys(parsedBodyContent).length] = element.value;
				});
	...
	renderTemplate (template) {
		...
		switch (template.type) {
			...
			case 'toast':
				...
				setTimeout(e => {
					this.unrender()
					try { this.onconfirm(); } catch (e) { null }
	...
	addBtnListeners() {
		...
		if (this.template.type == 'notification') {
			...
			this.self.querySelector('.global.close').addEventListener('click', e => {
				try{this.oncancel()} catch (e) {null}
				this.unrender();
			})
